        ...
        try {
            ...
            while(true){
                ...
                if(out.checkError() || Thread.interrupted()){
                    if(selectedCoupon!=null && selectedCoupon.lock != null)selectedCoupon.lock.V();
        ...
        if(message.length() > 0) {
            if (message.charAt(0) == MessageTypes.Greet.Message) {
                ...
                if (correct) {
                    ...
                    if (c != null && c.lock != null) c.lock.V();
                }
                ...
                if (c != null && c.lock != null && c.lock.P()) {
                    selectedCoupon = c;
...
class ProducerThread implements Runnable{
    private Coupon generate() {
        ...
        return new Coupon(buffer.toString(),new Date(),problem,difficulty*10
                + "TL" + " discount for shoppings above " + difficulty*50 + " TL.",result,getTimeOfProblem(difficulty,problem,result));
    }
    ...
    private static int getTimeOfProblem(int difficulty, String problem, int result){
        int bonus = 0;
        ...
        if(resultAbs > 1200) bonus += 10;
        else if(resultAbs > 800) bonus += 8;
        else if(resultAbs > 400) bonus += 3;
